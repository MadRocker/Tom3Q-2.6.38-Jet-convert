/*
 * gt_s8000_ak4671_max9880.c  --  SoC audio for Samsung GT-S8000 (jet)
 *
...
/*
 * GT-S8000 AK4671 HiFi DAI opserations.
 */
static struct snd_soc_ops gt_s8000_hifi_ops = {
	.hw_params = gt_s8000_hifi_hw_params,
};

static int gt_s8000_mic_event(struct snd_soc_dapm_widget *w,
...
static const struct snd_soc_dapm_widget gt_s8000_dapm_direct_widgets[] = {
	...
	SND_SOC_DAPM_MIC("Main Mic", gt_s8000_mic_event),
	SND_SOC_DAPM_MIC("Sub Mic", gt_s8000_mic_event),
	SND_SOC_DAPM_MIC("Jack Mic", gt_s8000_mic_event),
 ...
 */
static int gt_s8000_ak4671_init(struct snd_soc_pcm_runtime *rtd)
{
...
static int gt_s8000_max9880_init(struct snd_soc_dapm_context *dapm)
{
	int err;
...
static struct platform_device *gt_s8000_snd_device;
...
static int __init gt_s8000_init(void)
{
	...
	if (!machine_is_gt_s8000()) {
		pr_info("Only GT-S8000 is supported by this ASoC driver\n");
		return -ENODEV;
	}

	gt_s8000_snd_device = platform_device_alloc("soc-audio", -1);
	if (!gt_s8000_snd_device)
		return -ENOMEM;

	platform_set_drvdata(gt_s8000_snd_device, &gt_s8000);

	ret = platform_device_add(gt_s8000_snd_device);
	if (ret) {
		platform_device_put(gt_s8000_snd_device);
		return ret;
	}
	...
	if (ret) {
		dev_err(&gt_s8000_snd_device->dev,
...
err_unregister_device:
	platform_device_unregister(gt_s8000_snd_device);

	return ret;
}

static void __exit gt_s8000_exit(void)
{
...
module_exit(gt_s8000_exit);

/* Module information */
MODULE_AUTHOR("Tomasz Figa <tomasz.figa at gmail.com>");
MODULE_DESCRIPTION("ALSA SoC AK4671 max9880 GT-S8000");
MODULE_LICENSE("GPL");
